...
class TestDeliver(unittest.TestCase):
    ...
    def test_quarantine_error(self):
        quarantine_response = "Bad Request response from sdx-deliver: None"
        with patch('app.deliver.post') as mock_post:
            mock_post.return_value = self.r
    ...
    def test_retry_error(self):
        retry_response = "Bad response from sdx-deliver: None"
        with patch('app.deliver.post') as mock_post:
            mock_post.return_value = self.r
    ...
    @patch("app.deliver.requests.post")
    ...
    @patch('app.deliver.requests')
    def test_post_connection_exception(self, mock_request):
        mock_request.post.side_effect = ConnectionError()
        filename = "seft_survey"
        seft_message = {
            'tx_id': '0f534ffc-9442-414c-b39f-a756b4adc6cb',
            'filename': 'seft_survey'
